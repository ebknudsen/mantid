...
void Q1D2::exec() {
  ...
  if(m_dataWS) fprintf(stderr, "DetBankWorkspace %lu %lu\n", m_dataWS->getNumberHistograms(), m_dataWS->blocksize());
  if(waveAdj) fprintf(stderr, "WavelengthAdj %lu %lu\n", waveAdj->getNumberHistograms(), waveAdj->blocksize());
  if(pixelAdj) fprintf(stderr, "PixelAdj %lu %lu\n", pixelAdj->getNumberHistograms(), pixelAdj->blocksize());
  if(wavePixelAdj) fprintf(stderr, "WavePixelAdj %lu %lu\n", wavePixelAdj->getNumberHistograms(), wavePixelAdj->blocksize());
  if(qResolution) fprintf(stderr, "QResolution %lu %lu\n", qResolution->getNumberHistograms(), qResolution->blocksize());
  ...
  if (communicator().size() > 1) {
    ...
    if(communicator().rank() == 0) {
      ...
      communicator().send(0, tag, detIds.size());
      communicator().send(0, tag, detIds.data(), detIds.size());
    }
    // TODO detector IDs
  }
